...
def main():
    ...
    ivars = []
    weights = []
    for i,thing_id in tqdm(enumerate(lines_of_sight.keys()[:num_sightlines]), total=num_sightlines):
    ...
    wave_bins = get_fiducial_wavelength(np.arange(num_wave_obs + 1) - 0.5, coeff1=coeff1)
    wave_bin_centers = get_fiducial_wavelength(np.arange(num_wave_obs), coeff1=coeff1)
