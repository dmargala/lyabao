...
def main():
    ...
    forest_flux = np.ma.MaskedArray(
        forest_skim['flux'][:], mask=forest_skim['mask'][:])
    forest_ivar = np.ma.MaskedArray(
        forest_skim['ivar'][:], mask=forest_skim['mask'][:])
    ...
    param_coefs = np.tile(
        np.vstack(
            (
                np.ones(num_forest_waves),
                log_forest_wave_ratio
            )
        ).reshape((-1,), order='F'),
        num_forests)

    param_rows = np.repeat(
        np.arange(num_forests*num_forest_waves), num_params)

    param_cols = np.vstack(
        (
            np.repeat(np.arange(num_forests)*num_params, num_forest_waves),
            np.repeat(np.arange(num_forests)*num_params + 1, num_forest_waves)
        )
    ).reshape((-1,), order='F')
    ...
    print('Param coef shapes: {} {} {}'.format(
        param_coefs.shape, param_rows.shape, param_cols.shape))
    ...
    print('Continuum coef shapes: {} {} {}'.format(
        cont_coefs.shape, cont_rows.shape, cont_cols.shape))
    ...
    print('Model coef shapes: {} {} {}'.format(
        model_coefs.shape, model_rows.shape, model_cols.shape))

    model_shape = (
        num_forests*num_forest_waves,
        num_forest_waves + num_params*num_forests
    )

    model_matrix = scipy.sparse.csc_matrix(
        (model_coefs, (model_rows, model_cols)),
        shape=model_shape
    )
    ...
    print('y shape, num masked pixels: {} {}'.format(
        model_y.shape, np.sum(model_y.mask)))
    ...
    outfile.create_dataset('continuum_wave', data=forest_wave, **dataset_kwargs)
    outfile.close()
    ...
    def draw_example(i, **kwargs):
        ...
        plt.plot(
            forest_wave,
            forest_norm[i]*params_a[i]*np.exp(params_b[i]*log_forest_wave_ratio)*continuum,
            **kwargs
        )
    ...
    mask_params = (
        (params_a > .01) & (params_a < 100) &
        (params_b > -20) & (params_b < 20)
    )

    print ('Number with okay params: {:d}'.format(np.sum(mask_params)))
    delta_flux_mean = ma.average(
        delta_flux_obs[mask_params],
        axis=0,
